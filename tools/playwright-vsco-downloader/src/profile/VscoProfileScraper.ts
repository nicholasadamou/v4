...
import type { ImageEntry, ImageData } from "../types/index.js";
 ...
 */
export class VscoProfileScraper {
  ...
  async scrapeProfile(
    username: string,
    maxImages: number = 0
  ): Promise<VscoProfileResult> {
    const page = await this.browserManager.getPage();
    const profileUrl = `https://vsco.co/${username}`;

    console.log(chalk.blue(`🔍 Scraping VSCO profile: ${username}`));
    console.log(chalk.gray(`   Profile URL: ${profileUrl}`));
    ...
    try {
      ...
      const result: VscoProfileResult = {
        ...profileMetadata,
        username,
      ...
      console.log(chalk.green(`✅ Successfully scraped ${images.length} images from @${username}`));
      return result;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      console.error(chalk.red(`❌ Failed to scrape profile @${username}: ${errorMessage}`));
      throw error;
    }
  }
   ...
   */
  private async navigateToProfile(page: Page, profileUrl: string, username: string): Promise<void> {
    console.log(chalk.blue(`🌐 Navigating to profile: @${username}`));
    ...
    try {
      ...
      try {
        ...
        if (this.config.get("debug")) {
          console.warn(chalk.yellow(`⚠️  Could not check page content: ${pageError}`));
        }
      }
      throw new Error(`Failed to load VSCO profile @${username}`);
    }

    console.log(chalk.green(`✅ Profile page loaded: @${username}`));
  }
   ...
   */
  private async extractProfileMetadata(page: Page, username: string): Promise<Partial<VscoProfileResult>> {
    try {
      const metadata: Partial<VscoProfileResult> = {};
      ...
      if (this.config.get("debug")) {
        console.warn(chalk.yellow(`⚠️  Could not extract profile metadata: ${error}`));
      }
      return {};
    }
  }
  ...
  private async extractImages(
    ...
    username: string,
    maxImages: number = 0
  ): Promise<VscoImageInfo[]> {
    console.log(chalk.blue(`📸 Extracting images from @${username}'s profile`));

    const images: VscoImageInfo[] = [];
    let attempts = 0;
    ...
    try {
      ...
      console.log(chalk.blue(`   Found VSCO gallery container`));
      ...
      try {
        ...
        console.log(chalk.blue(`   Found at least one MediaThumbnail image`));
      } catch (e) {
        console.log(chalk.yellow(`   ⚠️  No MediaThumbnail images found initially`));
      }
      ...
      while (true) {
        ...
        for (const imageInfo of currentImages) {
          ...
          if (!isDuplicateById && !isDuplicateByUrl) {
            ...
            if (this.config.get("debug")) {
              console.log(chalk.green(`   Added new image: ${imageInfo.id} (${imageInfo.url?.substring(0, 50)}...)`));
            }

            // Check if we've reached the limit
            if (maxImages > 0 && images.length >= maxImages) {
              console.log(chalk.blue(`   Reached image limit: ${maxImages}`));
              return images.slice(0, maxImages);
            }
          } else {
            if (this.config.get("debug")) {
              console.log(chalk.yellow(`   Skipped duplicate image: ${imageInfo.id} (duplicate by ${isDuplicateById ? 'ID' : 'URL'})`));
            }
          }
        }

        console.log(chalk.blue(`   Currently have ${images.length} images`));
        ...
        if (!isLoadMoreVisible) {
          console.log(chalk.blue(`   No more "Load More" button found - reached end`));
          break;
        }

        if (attempts >= maxLoadMoreAttempts) {
          console.log(chalk.yellow(`   Reached maximum load attempts (${maxLoadMoreAttempts})`));
          break;
        }

        // Click the Load More button
        console.log(chalk.blue(`   Clicking "Load More" button (attempt ${attempts + 1})...`));
      ...
      console.log(chalk.green(`✅ Extracted ${images.length} images from @${username}`));
      return images;

    } catch (error) {
      console.error(chalk.red(`❌ Error extracting images: ${error}`));
   ...
   */
  private async extractCurrentImages(page: Page, username: string): Promise<VscoImageInfo[]> {
    const images: VscoImageInfo[] = [];
    ...
    try {
      ...
      let mediaThumbnails: any[] = [];
      let usedSelector = '';

      for (const selector of selectors) {
        const elements = await page.$$(selector);
        if (this.config.get("debug")) {
          console.log(chalk.blue(`   Selector '${selector}': Found ${elements.length} elements`));
        }
      ...
      console.log(chalk.blue(`   Using selector '${usedSelector}': Found ${mediaThumbnails.length} img elements`));
      ...
      if (this.config.get("debug")) {
        console.log(chalk.blue(`   MediaThumbnail figures in gallery: ${mediaThumbnailFigures.length}`));
      }
      ...
      for (let i = 0; i < mediaThumbnails.length; i++) {
        ...
        try {
          ...
          if (this.config.get("debug")) {
            console.log(chalk.gray(`     Image ${i + 1} src: ${src ? src.substring(0, 150) : 'no src'}`));
            if (srcset) {
              console.log(chalk.gray(`     Image ${i + 1} srcset: ${srcset.substring(0, 150)}...`));
            }
          }
          ...
          if (src && src.includes('vsco')) {
            ...
            if (imageInfo) {
              images.push(imageInfo);
              if (this.config.get("debug")) {
                console.log(chalk.green(`     ✅ Successfully extracted image ${i + 1}: ${imageInfo.id}`));
              }
            } else {
              if (this.config.get("debug")) {
                console.log(chalk.yellow(`     ⚠️  Failed to extract info for image ${i + 1}`));
              }
            }
          } else {
            if (this.config.get("debug")) {
              console.log(chalk.yellow(`     ⚠️  Image ${i + 1} doesn't contain 'vsco' in src`));
            }
          }
        } catch (error) {
          if (this.config.get("debug")) {
            console.warn(chalk.yellow(`⚠️  Error extracting image ${i + 1} info: ${error}`));
          }
        }
      }
    } catch (error) {
      if (this.config.get("debug")) {
        console.warn(chalk.yellow(`⚠️  Error finding MediaThumbnail images: ${error}`));
      }
    }
  ...
  private async extractImageInfo(
    ...
    try {
      ...
      try {
        width = await imgElement.evaluate((el: HTMLImageElement) => el.naturalWidth);
        height = await imgElement.evaluate((el: HTMLImageElement) => el.naturalHeight);
      } catch (e) {
      ...
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Final URLs - Original: ${originalUrl}, Thumbnail: ${thumbnailUrl}`));
      }

      const result: VscoImageInfo = {
        id: imageId,
        url: originalUrl || src,
      ...
      if (this.config.get("debug")) {
        console.warn(chalk.yellow(`⚠️  Could not extract image info: ${error}`));
      }
      return null;
    }
  }
  ...
  private extractImageIdFromUrl(url: string): string | null {
    try {
      ...
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Extracting ID from URL: ${url}`));
      }
      ...
      for (const pattern of patterns) {
        ...
        if (match && match[1]) {
          if (this.config.get("debug")) {
            console.log(chalk.gray(`     Matched pattern, extracted ID: ${match[1]}`));
          }
          return match[1];
        }
      }
      ...
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     No pattern matched for URL`));
      }
      return null;
    } catch (error) {
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Error extracting ID: ${error}`));
      }
      return null;
    }
  }
   ...
   */
  private generateOriginalUrl(url: string): string | null {
    try {
      ...
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Original URL processing: ${url}`));
      }
      ...
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Processed URL: ${processedUrl}`));
      }

      return processedUrl;
    } catch (error) {
      if (this.config.get("debug")) {
        console.log(chalk.gray(`     Error processing URL: ${error}`));
      }
      return null;
    }
  }
  ...
  getImageEntries(profileResult: VscoProfileResult): ImageEntry[] {
    return profileResult.images.map(image => {
      ...
      return [`${profileResult.username}/${image.id}`, imageData] as ImageEntry;
    });
  }
